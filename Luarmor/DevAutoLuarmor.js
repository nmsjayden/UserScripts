...
(function () {
    ...
    const POPUP_BLACKLIST = ["oundhertobeconsist.org", "ghabovethec.info", "undaymidydles.org",];
    ...
    const SCRIPT_BLOCKED_DOMAINS = [
        ...
        "rubiconproject.com", "openx.net", "pubmatic.com", "adf.ly", "undaymidydles.org", "ghabovethec.info",
    ];
    ...
    async function callBypassApi(url, apiKey) {
        try {
            ...
            if(data && data.success === true && typeof data.result === "string") {
                log("API bypass: success → " + data.result);
                return data.result;
            }
    ...
    for (let attempt = 1; attempt <= MAX_API_RETRIES; attempt++) {
        setAction(`API bypass: attempt ${attempt}/${MAX_API_RETRIES}…`);
        const bypassedUrl = await callBypassApi(capturedUrl, apiKey);

        if (bypassedUrl) {
            log("API bypass: got result → " + bypassedUrl.slice(0, 60));
            setAction("API bypass: redirecting…");
            await sleep(humanDelay(300, 100, 200, 500));
    ...
    log("API bypass: all retries failed");
    if (opts.apiFallback) {
        log("API bypass: falling back to normal click");
...
#dotWrap{width:${Math.round(22*F)}px;height:${Math.round(22*F)}px;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
...
.discord-btn{width:${Math.round(22*F)}px;height:${Math.round(22*F)}px;border-radius:4px;background:${t.hdrBtn};border:1px solid ${t.hdrBtnBorder};cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .15s,border-color .15s;flex-shrink:0;pointer-events:all;text-decoration:none;}
.discord-btn:hover{background:#5865f2;border-color:#7289da;}
.discord-btn svg{width:${Math.round(13*F)}px;height:${Math.round(13*F)}px;fill:${t.textDim};transition:fill .15s;}
.discord-btn:hover svg{fill:#ffffff;}
...
input[type="checkbox"]{width:11px;height:11px;accent-color:${t.accent};cursor:pointer;flex-shrink:0}
...
input[type="password"],input[type="text"]{background:${t.bg3};border:1px solid ${t.border};border-radius:3px;color:${t.text};font-family:'JetBrains Mono',monospace;font-size:${Math.round(9.5*F)}px;padding:2px 6px;outline:none;width:130px;transition:border-color .15s,box-shadow .15s;}
input[type="password"]:focus,input[type="text"]:focus{border-color:${t.accent};box-shadow:0 0 0 1px ${t.accentBorder};}
input[type="password"]::placeholder,input[type="text"]::placeholder{color:${t.textDim};opacity:.6;}
...
function isBlocked() {
    ...
    // Linkvertise callback failure
    const swalHtml = q("#swal2-html-container");
    if (swalHtml && swalHtml.style.display !== "none" && swalHtml.textContent.includes("Linkvertise callback is invalid")) {
        setAction("Bypass Failed — retrying…");
        setDot("error");
        // Reset so it tries the step again
        clickedStepsThisLoad.delete(lastClickedStep);
        lastClickedStep = lastClickedStep - 1;
        phase = "idle";
        clicking = false;
        return false; // don't block, let it retry
    }
    ...
    const newBtn = qid("newkeybtn");
    const btn = qid("nextbtn");

    // If there's already a next button, let normal flow handle it
    if (canClick(btn)) {
        // fall through to normal tick logic below
    } else if (canClick(newBtn)) {
